...
def test_default_is_parallel(_isolated):
...
def test_set_back_to_solo_still_returns_parallel(_isolated):
    exec_mode.set_current(exec_mode.PARALLEL)
...
def test_corrupt_file_returns_parallel(_isolated):
    exec_mode.path().write_text("{not json", encoding="utf-8")
    assert exec_mode.current() == exec_mode.PARALLEL


def test_non_dict_json_returns_parallel(_isolated):
