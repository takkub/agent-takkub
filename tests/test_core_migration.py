...
# ---------------------------------------------------------------------------
# #362 2.0.0 flip — rollback path (TAKKUB_V2_AUTHORITY=0 escape hatch)
# ---------------------------------------------------------------------------


def test_rollback_preserves_data_written_while_v2_authoritative(tmp_path, monkeypatch):
    """#362 rollback-path proof: a value written through a real V1 writer
    while `TAKKUB_V2_AUTHORITY=1` (the 2.0.0 default) must still be there
    after (a) flipping the escape hatch to `=0` and (b) running a full
    `MigrationEngine.rollback()` — which deletes `v2/` outright (see
    `MigrationEngine.rollback`'s own docstring, #350). This only holds
    because `role_models._save()` always writes the V1 file directly first;
    dual-write to v2/ is a mirror, never the only copy — so deleting the
    mirror can never lose data V1 doesn't already have."""
    ...
    monkeypatch.setattr(
        "agent_takkub.core.storage.dual_write._effective_data_home", lambda dh=None: data_home
    )
    monkeypatch.setattr(
        "agent_takkub.core.storage.v2_authority._effective_data_home", lambda dh=None: data_home
    )

    monkeypatch.delenv("TAKKUB_V2_AUTHORITY", raising=False)  # unmigrated: reads V1 either way
    role_models.set_model("backend", "codex", "gpt-5.5")
    ...
    # Now V2-authoritative (2.0.0 default) — re-pin the model; dual-write
    # mirrors it into the same v2/models/aliases.json target this ladder
    # step just wrote, and the read comes back from that mirror.
    ...
    # Escape hatch + full rollback: v2/ is gone, layout is back to "v1"...
    monkeypatch.setenv("TAKKUB_V2_AUTHORITY", "0")
    rollback_reports = engine.rollback()
    assert all(r.ok for r in rollback_reports), [(r.step_id, r.summary) for r in rollback_reports]
    assert layout_state(data_home) == "v1"

    # ...but the re-pin made while V2 was authoritative is still there, read
    # straight off V1 — nothing was lost by deleting the v2/ mirror.
    assert role_models.model_for("backend", "codex") == "gpt-5.6"
