...
def test_resolve_account_for_legacy_fallback_carries_no_env_override(tmp_path):
    ...
    from agent_takkub.core.accounts.facade import resolve_account_for
    from agent_takkub.core.accounts.registry import AccountPoolRegistry
    from agent_takkub.core.providers.plan import account_env_overrides
    from agent_takkub.core.storage.jsonl_store import JsonlStore as _JsonlStore
...
def test_resolve_account_for_pool_account_keeps_config_dir(tmp_path):
    """Counterpart: a REAL V2 pool account is the one thing that may override
    env, so its config_dir must still come through untouched."""
    from agent_takkub.core.accounts.facade import resolve_account_for
    from agent_takkub.core.accounts.registry import AccountPoolRegistry, AccountRegistry
    from agent_takkub.core.providers.plan import account_env_overrides
    from agent_takkub.core.storage.jsonl_store import JsonlStore as _JsonlStore
    ...
    accounts.upsert(
        ProviderAccount(id="pool-a1", provider_id="claude", config_dir=str(tmp_path / "pool-home"))
    )
    pools.upsert(
        AccountPool(
            id="p1",
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        account = resolve_account_for("claude", "some-project")
    assert account.config_dir == str(tmp_path / "pool-home")
    assert account_env_overrides("claude", account) == {
        "CLAUDE_CONFIG_DIR": str(tmp_path / "pool-home")
    }
