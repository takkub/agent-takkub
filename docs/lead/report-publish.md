...
### รูปภาพ
- รองรับ PNG/JPG/GIF/WebP → แปลงเป็น **JPEG q80 กว้างสุด 1440px** ฝังเป็น base64 data URI (25 รูป ≈ 2.4MB) · ปรับได้ด้วย `--max-width <px>` (#676)
- `images.txt` รูปแบบ `name|path` หนึ่งบรรทัดต่อรูป (`#` = comment)
- **placeholder ใน content.html (#675/#676):** `{{img:name}}` = ค่า src เปล่าๆ (แบบเดิม) · `{{figure:name|คำบรรยาย}}` = figure พร้อม figcaption (คำบรรยายที่ดีบอกว่าในภาพกำลังเกิดอะไร ไม่ใช่ชื่อไฟล์) · `{{imgpair:desk|mobile|คำบรรยาย}}` = ภาพคู่คอม/มือถือ ป้ายกำกับ+เรียงลงแนวตั้งบนจอแคบให้เอง
- **lightbox ได้ฟรีทุกรูปใน `<figure>`** — กดขยายเต็มจอ พื้นหลังมืด ปิดด้วย Esc/กดที่ว่าง ซูมนิ้วบนมือถือได้ ไม่ต้องเขียน JS เอง (#675)
- **ไฟล์เกิน 4MB (หรือ `--max-bytes`) = เตือนดังๆ** ให้ลดคุณภาพ/แยกไฟล์ — **ห้ามตัดหัวข้อทิ้งเงียบๆ** (#676)
- **`--lint` ตรวจได้แต่ข้อความ มองไม่เห็นในพิกเซลของรูป** — ภาพทุกใบต้องเปิดดูด้วยตาเองก่อนส่งลูกค้า (เตือนเพิ่ม: การันตีเวลา, คำว่า "อัตโนมัติ", อีเมล/บัญชีทดสอบ/host:port ภายใน — เป็น warn ไม่ block)
