...
takkub lock <name> [--wait <s>] [--note "..."]          # (#430, ทุก role) advisory lock ต่อ project รอบ shared resource (build dir/db) — ไฟล์ใน runtime/locks/<project>/ ใช้ได้แม้ cockpit ค้าง · ติด = exit 3 + บอกว่าใครถือ · `takkub lock --list` ดูทั้งหมด · `takkub unlock <name>` (holder เท่านั้น; lead `--force`) · หมดอายุเอง 30 นาที
takkub kill --role <role> [--pid N]                    # (#430, lead) ฆ่า process ใต้ pane นั้นทั้ง tree (หรือเฉพาะ --pid ที่อยู่ใต้ pane) ไม่ต้องไล่ PID เอง · ลง events.log `pane_children_killed`
takkub spawn-service --name <n> -- <cmd> [args]         # (#429, ทุก role) start service ให้อยู่รอด `done`/`close` — cockpit spawn นอก job/tree ของ pane (Windows DETACHED+BREAKAWAY_FROM_JOB / POSIX setsid) · log runtime/services/<project>/<n>.log · `--list` ดู · lead หยุดด้วย `takkub service-stop --name <n>` · ⚠️ git-bash/WSL: MSYS แปลง arg ที่ขึ้นต้น `/` เป็น Windows path (`/c` → `C:/Program Files/Git/c`) ทำ args เพี้ยน (#645) — ใช้ `--cmd-file <file>` (1 บรรทัด 1 argv) หรือ `MSYS_NO_PATHCONV=1`
...
takkub assign --role frontend --shards 5 "<5 ชิ้นอิสระ>"  # (#641) DEFAULT fan-out = เปิด pane เดียว แล้ว pane ยิง native subagent 5 ตัวคู่ขนานเอง (claude Agent / codex spawn_agent / agy run_subagent / opencode task) → done ครั้งเดียว · ห้ามยิง frontend#1..#5 แยก pane เอง
takkub assign --role backend --shards 4 --fanout pane "<task>"      # (#641) บังคับแบบเดิม N shard panes (backend#1…#4 · env TAKKUB_SHARD/_TOTAL) · --fanout subagent = บังคับแบบใหม่ error ถ้า provider/role ทำไม่ได้
...
takkub qa-gate --auto                                  # #436: เลือก tier จาก git diff + พิมพ์เหตุผล — style/asset/i18n/md ล้วน = ไม่รัน test เลย (Node: typecheck เท่านั้น) · module logic = targeted tests ที่ map ได้ (Node #482: `jest --findRelatedTests`/`vitest related` เฉพาะไฟล์ที่แก้ ถ้ามี jest/vitest จริง — ไม่มี หรือมีแต่ script `verify` รวมทุกอย่างเป็นก้อนเดียว = narrow ไม่ได้ widen เป็น full พร้อมบอกเหตุผลตรงๆ, ไม่เงียบ; typecheck รันเต็มเสมอไม่ว่ากรณีไหน · targeted tier ที่ narrow ได้จริงไม่ต้องรอ full-gate lock เหมือน Python) · api/auth/schema/shared/tooling = full · clean tree ไม่มี diff เลย (#482): เทียบ `origin/<default-branch>..HEAD` ก่อน (จับ commit ที่ยัง unpushed ทั้งหมด ไม่ใช่แค่ commit ล่าสุด) ไม่มี remote = fallback `HEAD~1..HEAD` ไม่มีทั้งคู่ = tier "none" แนะนำรัน full ตรงๆ ในผลลัพธ์ ไม่ใช่เงียบข้ามแบบ style-only — **#485: รันโดย qa pane ครั้งเดียวตอนจบ batch** (หลัง qa เทสงานที่แก้ผ่านแล้ว ก่อน merge/push) — specialist ห้ามรัน gate เองก่อน `takkub done` (gate ต่อทุก done ทำเครื่อง user ค้าง)
...
takkub qa-gate            (ใน Node project)          # #329/#368: delegate ไป check ของโปรเจคนั้น — ลำดับ: `verify` script › `typecheck` script › `tsc -p tsconfig.json --noEmit` (root ไม่มี → ทุก workspace package ที่มี tsconfig) › `test` › eslint · package manager จาก lockfile (pnpm/yarn/bun/npm ห้าม hardcode) · typecheck รันก่อนและรันเต็มเสมอ (vitest/esbuild ไม่เช็ค type → false-PASS แล้วไปแดง CI คือสิ่งที่ #368 ปิด) · explicit `--targeted <path...>` (CLI flag ตรงๆ) ไม่ narrow อะไรบน Node (gate บอกตรงๆ) — คนละอันกับ `--auto`'s module-logic tier ที่ narrow ได้จริงด้วย jest/vitest related-files mode ข้างบน (#482)
...
takkub issue new ... --severity <s>                     # **issue = บั๊กเท่านั้น** (user 2026-09-25) — งานถอด/refactor/feature/cleanup ลง `takkub backlog add` แทน · (#297) cockpit เปิด issue ให้อัตโนมัติด้วยเมื่อเจอ crash หรือสัญญาณผิดปกติใน events.log — เปิดเป็น default ปิดที่ Settings → Performance หรือ TAKKUB_AUTO_ISSUE=0 · ส่งแค่ชนิด event + จำนวน + version + platform ไม่ส่งเนื้อ task/path/token
