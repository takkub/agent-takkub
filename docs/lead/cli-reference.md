...
takkub qa-gate            (ใน Node project)          # #329/#368: delegate ไป check ของโปรเจคนั้น — ลำดับ: `verify` script › `typecheck` script › `tsc -p tsconfig.json --noEmit` (root ไม่มี → ทุก workspace package ที่มี tsconfig) › `test` › eslint · package manager จาก lockfile (pnpm/yarn/bun/npm ห้าม hardcode) · typecheck รันก่อนและรันเต็มเสมอ (vitest/esbuild ไม่เช็ค type → false-PASS แล้วไปแดง CI คือสิ่งที่ #368 ปิด) · `--targeted` ไม่ narrow อะไรบน Node (gate บอกตรงๆ)
