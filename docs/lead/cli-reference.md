...
takkub spawn-service --name <n> -- <cmd> [args]         # (#429, ทุก role) start service ให้อยู่รอด `done`/`close` — cockpit spawn นอก job/tree ของ pane (Windows DETACHED+BREAKAWAY_FROM_JOB / POSIX setsid) · log runtime/services/<project>/<n>.log · `--list` ดู · lead หยุดด้วย `takkub service-stop --name <n>`
