...
- `assign`, `_assign_dispatch`, `_assign_with_worktree`, `request_restart`, `_tag_pane_worktree`, `_finalize_worktree`, `send`, `close`, `done`, `set_session_goal`, `clear_session_goal`, `get_session_goal`, `_apply_session_goal`, `toggle_provider`, `set_plan_tier`, `close_all_teammates`, `harvest_info`, `task_show_info`, `_uncommitted_warning`, `_check_uncommitted_async`, `_save_decision_note`, `consume_pane_hook`, `consume_session_report`, `_condense_done_note`, `_build_verify_fail_handoff`, `_build_blocked_handoff` (#296 — BLOCKED branch: no role, no fix loop)
- `set_exec_mode` removed (#515 Settings diet) — exec mode derives from the team preset (`exec_mode.is_parallel(project)`) instead of a standalone status-bar toggle; `set_auto_resume`/`execModeChanged`/`autoResumeChanged` removed the same way (auto-resume is unconditionally on, #514/#515)
