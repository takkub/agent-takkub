...
ใหม่ — mixin `CoreV2SettingsMixin`) แทนที่จะเติมเข้า `src/agent_takkub/settings_window.py` ตรงๆ
...
`src/agent_takkub/settings_window.py` import ใช้อยู่แล้ว) จำเป็นสำหรับให้ flag fallback + Scheduler
...
- **Backpressure panel เป็น estimate ไม่ใช่ live state** — `classify()` ต้องการ
  ...
  `src/agent_takkub/settings_window.py` ประกาศชัดในหัวไฟล์ตัวเองว่า "MUST NOT import app or cli"
...
`cockpit_theme` token (ไม่มี hex literal ใหม่ใน `src/agent_takkub/settings_core_v2.py`), ปุ่ม
