...
`src/agent_takkub/config.py` untouched — no constant added or changed (the layout builder reads
`config.DATA_HOME`/`config.SETTINGS_HOME` live, per plan §3.4's own rule that
`src/agent_takkub/core/storage/paths.py` must never declare a new home).

## 8b-1 — `src/agent_takkub/core/storage/layout.py`
...
— mirrors `src/agent_takkub/disk_usage.py`'s own `data_home` parameter convention, so a test
...
to `src/agent_takkub/disk_usage.py`, so `takkub prune`'s behavior is completely unchanged by
...
- **Credential reference step (6) only resolves `claude`/`codex`/`opencode`**
  ...
  codebase yet (same posture `src/agent_takkub/provider_bootstrap.py`/`src/agent_takkub/doctor.py` already
