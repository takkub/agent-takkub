...
- **4 ingest adapters** (claude/codex/gemini/opencode) — the same 4 providers
  ...
  Each **WRAPS** the existing reader (`chatlog_scanner.py`, `codex_helper.py`, `gemini_helper.py`,
  `opencode_helper.py`, `token_meter.py`'s `find_session_by_uuid`/`find_latest_session`) — none of
...
- **Remote mirror read-through** (§6d): `remote/notify.py`'s ONE touch point, fail-open, flag off
  by default.
...
1. **kimi/cursor have no ingest adapter** — same gap `remote/notify.py` already has
...
4. **Codex/gemini record parsing is a deliberate, documented DUPLICATE**, not a reuse, of
   `remote/notify.py`'s `_codex_record_message`/`_antigravity_record_message`/
   `_gemini_record_messages` — `core.*` cannot import `remote/notify.py` (it imports
...
- **Ingest adapters expose a plain `(resolve_source, read_new)` module-level pair**, not a class —
  matches how `remote/notify.py`'s own `_HistoryScanner` treats its callables, and keeps
