...
- ✅ **Codex CLI early-crash on Windows under cockpit env** — FIXED 2026-05-21 via Experiment 2 (commit cf6529b). Root cause: `_PANE_ENV_ALLOWLIST` (commit ab1ff5f) stripped `COMSPEC` from the teammate env. Node.js `child_process` on Windows needs `COMSPEC` to locate `cmd.exe` when shelling out — without it, the `codex_apps` MCP server boot hung silently after the splash banner. Symptom was first-spawn-only because auto-respawn re-ran with the same broken env and got lucky on retry (timing-dependent). Hypothesis sequence: instrumentation (commit c242ad6, dump to `runtime/codex_crash_dumps/`) → gemini hypothesis ranking (commit 425d158, top 2 tied at score 14: MCP race vs env gap) → Experiment 2 added `COMSPEC`, `USERDOMAIN`, `SESSIONNAME`, `LOGONSERVER` to the allowlist → smoke test after cockpit restart: codex first-spawn survived MCP boot and completed its task. **Verified live** 2026-05-21 (see commit cf6529b + session run). Instrumentation kept in place as a regression guard — next early-crash will still dump diagnostics.
