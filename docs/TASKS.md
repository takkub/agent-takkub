...
- ⚪ **Backend done-without-commit protocol bug** — pane รายงาน `takkub done` ครบทุก phase แต่ไม่ `git commit` ก่อน → Lead เห็น working tree มี modified files ค้าง ต้อง commit แทน (เกิด 2026-05-21 ตอน hardening round 2 task spec บังคับ commit per task แต่ pane ข้าม). คาดว่า claude pane มอง "task เสร็จ" = "code เขียนเสร็จ + tests pass" โดยลืม commit step. Fix sketches: (a) task spec แทรก "git status MUST be clean before takkub done" ทุก phase (b) orchestrator gate `takkub done` ถ้า `git status --porcelain` ไม่ว่าง → reject + นานๆ retry
...
- ⚪ **Resume bleed: teammate panes inherit Lead's chat history** — `orchestrator._recent_exits` resume logic (`RESUME_WINDOW_SEC = 5*60`) passes `--continue` when a role respawns within 5 min into the same cwd. Problem: when a teammate (backend/qa/etc.) spawns in the project root where Lead is already running, claude resumes the LATEST chat for that cwd — which is Lead's. The teammate then acts on Lead's first task in history instead of the just-pasted assign. Workaround used 2026-05-21: spawn with cwd suffix `/.` so string-comparison at `orchestrator.py:1235` fails and resume is skipped. Proper fix: scope resume by `_exit_key` (project::role) only, NOT cwd — or store per-role session UUIDs and pass `--resume <uuid>` instead of `--continue`. Risk if unfixed: every fresh teammate task in project root will inherit wrong context.
- ⚪ **Codex CLI early-crash on Windows under cockpit env** — codex pane spawn ครั้งแรกตายเองภายใน ~50s ไม่มี error visible (transcript เหลือแค่ 2.6KB = ขึ้น banner แล้วเงียบ) เกิดซ้ำ 2026-05-21 ตอน review session. orchestrator auto-respawn สำเร็จในครั้งที่ 2 (ทำงานต่อได้) แต่ก่อน fix `auto_respawn_replay` (commit หลัง) task ที่ assign ครั้งแรกหายไปเลย → user เห็น dead pane. Hypotheses: (a) MCP server `codex_apps` boot blocks input handler ระหว่าง paste — race condition (b) `--ask-for-approval never -s workspace-write` + cockpit's env allowlist (commit ab1ff5f) ตัด env var บางตัวที่ codex CLI ต้องการ (c) PTY cols=110 rows=36 ไม่ match codex's expected terminal geometry → crash on redraw (d) `_paste_payload` ส่ง bracketed-paste sequence ขณะ codex ยัง boot MCP → codex parser confused. Investigate: ดู codex.exe exit code + stderr ก่อน respawn (ต้อง capture ใน PtySession.processExited), repro โดย spawn codex ซ้ำๆ ใน cockpit env ดู crash rate, เทียบ env ระหว่าง bare codex shell vs cockpit-spawned codex
