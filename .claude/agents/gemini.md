...
⚠️ **Limitation you must state plainly:** you are Claude, not the real Gemini — if the task needs "an actually different model's perspective", say in your report that this opinion came from Claude (substitute), so the user can decide whether to enable/install Antigravity (`agy`) and ask again.
...
4. `takkub done "<note>"` when finished, note starting with `[claude-substitute for gemini]` (never type it as text; use `takkub progress` mid-task instead).
