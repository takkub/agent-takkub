...
⚠️ **`graft` ผลลัพธ์ "no callers" / "no matching nodes" ไม่ใช่หลักฐานว่าโค้ดตายแล้ว** — พิสูจน์แล้วว่ามี false negative จริง (เช่น type-annotated instantiation `x: Type = Type()` ที่ graft ไม่นับเป็น call edge) ก่อนสรุปว่า symbol ไม่ถูกใช้ ต้อง grep cross-check เสมอ — สำคัญมากสำหรับ reviewer เพราะ "no callers" อาจถูกใช้เป็นหลักฐานว่าโค้ด dead ผิดๆ
...
⚠️ **Anchoring:** ได้ ranked list จาก graft มาแล้ว **ห้ามหยุดหาแค่นั้น** — ranked list ไม่ exhaustive ถ้าไม่เจอสิ่งที่ต้องการในผลลัพธ์ ต้อง grep ต่อเสมอ ก่อนสรุปว่าไม่มี — สำคัญมากสำหรับ reviewer เพราะข้อสรุป "ไม่มี" มักไปลงใน review verdict ตรงๆ

⚠️ **Staleness:** เพิ่งแก้ไฟล์นี้เอง (Edit/Write เมื่อกี้) → **ห้ามเชื่อ graft ทันทีสำหรับไฟล์นั้น** — graph รีเฟรชอัตโนมัติภายใน ~15 วินาที ไม่ใช่ instant ถ้าต้องคำตอบที่ชัวร์ ณ ตอนนี้ อ่านไฟล์จริงแทน

⚠️ **ไฟล์สร้างใหม่:** ไฟล์ที่คุณเพิ่งสร้างใหม่ (ไม่เคยอยู่ใน graph มาก่อน) graft จะยังไม่เห็นทันที — ผลว่างจากไฟล์ใหม่ **ไม่ใช่หลักฐานว่าไม่มี** อย่าสรุปแบบนั้น ให้ใช้ Glob/Grep แทนสำหรับไฟล์ที่เพิ่งสร้างจนกว่าจะมั่นใจว่า build รอบใหม่เสร็จแล้ว
