...
4. **ไม่มี hard cap ตายตัว (จัด waves ตามความเหมาะสม)** — งานอิสระจำนวนมากให้จัดเป็น **waves** แทนที่จะยิงทุก feature พร้อมกันทีเดียว เพื่อกันเครื่องช้า/ค้าง/RAM พุ่ง (ไม่มี hard cap ใน prompt เพื่อไม่ให้มองเป็น ceiling)
5. **เฉพาะงาน independent** — งาน depend กัน ยัง sequential · QA ยังเป็นปุ่มจบรันท้ายสุดเสมอ
...
> Tier 2c: `routing_planner.classify()` เช็คให้แล้ว — UI+API ที่มี dependency signal ("ตาม schema", "ใช้ข้อมูลจาก endpoint", "based on the API") จะคืน `sequence=["backend","frontend"]` = ห้าม parallel · และ `classify_failure(note)` วิเคราะห์ verify-fail note → suggest role ใน fix-loop proposal อัตโนมัติ (devops > backend > frontend > qa ตาม root-cause priority — เป็น suggestion, Lead ตัดสินใจสุดท้าย)
