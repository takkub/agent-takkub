...
- **critic** — Design Critic: รีวิว UI หลัง QA แคป screenshots, ส่งภาพให้ Gemini ดู, รวบรวมข้อเสนอ (เพิ่ม/ลบ/ปรับ) เขียน proposal markdown ส่งกลับ Lead
...
- **Cross-check claude's plan** — ถ้าสงสัย → ใส่ row `takkub assign --role codex "review this approach: <plan>"` ใน propose table คู่กับ implementation role (pane เสมอ ห้าม one-shot)
...
- **Brainstorm options** — `takkub assign --role gemini "3 ideas for X + tradeoffs"` (pane เสมอ ห้าม one-shot — user ต้องเห็นความคิด gemini สด)
...
- **หลัง QA smoke test เสร็จ + แคป screenshots** — fire critic เพื่อรีวิว UI หน้านั้น เสนอ เพิ่ม/ลบ/ปรับ
- **ก่อน ship feature ที่มี UI component ใหม่** — เป็น pre-ship gate (parallel กับ reviewer code review)
- **เปลี่ยน design / redesign** — critic ดูทั้ง flow + เสนอ heuristic (Nielsen 10) violations
- **User บ่นว่า UI งง / ใช้ยาก / น่าเบื่อ** — critic หา root cause + เสนอ remedy ที่ frontend implement ได้

**Pipeline (3 hops):**

```bash
# Hop 1: QA smoke + capture shots
takkub assign --role qa --cwd <web> "smoke /login → /dashboard · save shots to runtime/exports/\$(date +%F)/<project>/screenshots/"

# (รอ qa done event — Lead เห็น path ของ shot dir)

# Hop 2: critic + gemini parallel — gemini ขนานเพราะ critic ใช้ takkub send ให้ gemini ดูภาพ
takkub assign --role critic --cwd <web> "design review screenshots ที่ runtime/exports/<date>/<project>/screenshots/ — เสนอ เพิ่ม/ลบ/ปรับ" &
takkub assign --role gemini --cwd <web> "เตรียม view images ที่ critic จะส่งมาผ่าน takkub send"                                         &
wait

# (รอ critic done event — Lead อ่าน docs/design-review/<date>-<view>.md)

# Hop 3: ตัดสินใจ delegate frontend ทำตาม proposal ไหม
takkub assign --role frontend --cwd <web> "implement design-review proposals จาก docs/design-review/<date>-<view>.md — focus high-impact ก่อน"
```

**Cross-check rule:** routing_planner จะใส่ `gemini` เป็น cross_check ของ `critic` อัตโนมัติเมื่อ user พูดว่า "design review / UI review / รีวิว UI / รีวิวดีไซน์" → propose table มี 2 rows (critic + gemini) parallel
...
- **claude-obsidian**: wiki / hot cache / save commands (plugin ยังไม่ถูก load ผ่าน cockpit — 1.4.3 มี SessionStart hook ที่เคย crash panes; รอ smoke-test ก่อน enable)
- **ecc** (Engineer's Coding Companion): context-aware hooks (gateguard + cost-critical muted via env vars)
- **MCP servers** — browser MCPs force-injected + user MCPs จาก allowlist (obsidian-vault, postgres-pms) auto-merged ตอน boot (ดู section "Shared MCPs" ด้านล่าง)
...
### Shared MCPs — user-level inheritance

cockpit auto-merge MCPs เข้าทุก pane ผ่าน `runtime/shared-mcp.json` จาก 2 แหล่ง:

**1. Browser MCPs (force-inject, ชนะ conflict):**

| Server | Package | ใช้ตอนไหน |
|---|---|---|
| `playwright` | `@playwright/mcp@0.0.75` | smoke / UX / e2e tests ที่ Lead สั่งโดยตรง หรือ delegate ให้ QA |
| `chrome-devtools` | `chrome-devtools-mcp@0.26.0` | inspect runtime state ของ web app ที่กำลังเปิดอยู่ |

**2. User MCPs (allowlist-merge จาก `~/.claude.json`):**

`ensure_user_mcps()` รันตอน Orchestrator init อ่าน top-level `mcpServers` จาก `~/.claude.json` แล้ว merge เฉพาะ entries ที่อยู่ใน allowlist เข้า shared-mcp.json:

| MCP name | Default | เหตุผล |
|---|---|---|
| `obsidian-vault` | ✅ include | stdio, ไม่มี credentials |
| `postgres-pms` | ✅ include | stdio, db creds user ตั้งใจ |
| `pms` | ❌ skip | HTTP + Authorization bearer → security risk |
| ชื่ออื่น ที่มี `headers.Authorization` หรือ env `TOKEN/KEY/SECRET` | ❌ skip + log warning | credential-bearing |
| `playwright`, `chrome-devtools` | ❌ skip | browser MCP ของ cockpit ชนะ |

**opt-in pms:** set `TAKKUB_INCLUDE_PMS=1` ก่อน launch cockpit เพื่อ include `pms` (ทราบ risk: bearer token จะถูก write ลง `runtime/shared-mcp.json`)

Notes:
- Authorization header values → ไม่เคย log (redact ทุกกรณี)
- `~/.claude.json` อ่านไม่ได้ → log warning, skip silently (cockpit ไม่ crash)
- entries ที่ไม่ใช่ stdio/http/sse type → ถูก skip (claude.ai built-in connectors)

ทำไมต้อง inject ผ่าน cockpit ไม่ใช่ให้ claude อ่านจาก `~/.claude.json` โดยตรง:
- pane ทุกตัว spawn ด้วย `--setting-sources project,local` (กัน claude-obsidian SessionStart hook crash)
- flag นี้ block user-level `mcpServers` ไปด้วย → user config ใน `~/.claude.json` ไม่เห็นใน pane
- fix: merge entries เข้า `runtime/shared-mcp.json` ก่อน spawn → `--mcp-config` + `--strict-mcp-config` ทำให้ทุก pane เห็น MCPs เหมือนกันทุก project
...
## Auto-routing (clear-rec auto-fire, propose only when ambiguous)
...
**Default:** ถ้า Lead มี clear single-best recommendation → **fire ตรงๆ** พร้อมระบุ 1 บรรทัดว่าทำอะไร user แทรกแก้ระหว่างทางได้

**Propose-then-fire (เสนอ table แล้วรอ confirm)** เฉพาะเมื่อ:
- choice ambiguous จริง (2+ approach tradeoff ใกล้กัน Lead ตัดสินไม่ได้คนเดียว)
- ต้องการ user knowledge ที่ Lead ไม่มี (เช่น "ยังใช้ feature X อยู่ไหม?")
- irreversible / shared-state actions: `git commit`, `git push`, `git push --force`, delete branches/files นอก scratch, drop DB, ส่งข้อความออก external service

**ทำไมเปลี่ยน:** feedback user 2026-05-20 — propose-then-fire ทุกครั้งเพิ่ม friction โดยไม่ได้ value เพราะ user เลือก recommendation ของ Lead ตลอด
...
- ทุก `takkub assign --role <role>` (spawn pane) — **รวม codex และ gemini** — กิน slot, spawn ผิด role → ต้อง close/redo, spawn ผิด cwd → ทำงานผิด project
- ทุกการแตะไฟล์ใน BLOCKED_DIRS (project paths) — อยู่ใน policy เดิม

### ❌ ห้ามใช้ `takkub codex` / `takkub gemini` (one-shot)

CLI ของ 2 คำสั่งนี้ยังมีอยู่ แต่ **Lead ห้ามใช้เด็ดขาด** — user ต้องการเห็น codex/gemini ทำงานสดใน pane เหมือน teammate ปกติ ไม่ใช่ text reply เงียบๆ ที่ user มองไม่เห็น

แทนที่จะ:
```bash
# ❌ ห้าม: one-shot, ไม่ spawn pane
takkub codex "review approach: ..."
takkub gemini "3 ideas for X"
```

ใช้:
```bash
# ✅ ทำ: ใส่เป็น row ใน propose table คู่กับ implementation role
**แผน:**
| Role | Task | cwd |
| backend | implement /auth/logout — JWT blacklist + session cleanup | <api path> |
| codex   | review approach: POST /auth/logout. edge cases? | <api path> |
