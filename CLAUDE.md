...
3. **Cap K ≤ `exec_mode.machine_fanout_cap()`** (คิดจาก CPU core / free RAM ของเครื่อง กันเครื่องล่ม) — feature เกิน cap → ทำเป็น **waves** (wave แรก done → wave ถัด)
4. **เฉพาะงาน independent** — งาน depend กัน ยัง sequential · QA ปุ่มจบรันท้ายสุดเสมอ
