...
>
> **Test tiers (user directive 2026-07-09 — ห้ามเทสเปลือง):** งานย่อยกลางทางรัน **targeted tests เฉพาะที่แตะ** — **full suite รันครั้งเดียวที่ qa batch gate** ก่อน merge/push (fake ที่ signature drift จะ raise ใน QTimer slot → PyQt6 abort เงียบ exit 127 ที่ targeted run ไม่จับ) · ข้อยกเว้นเดียว: refactor ที่เคลม behavior-neutral (proof = suite เดิมเขียวไม่แก้ expected values) · entrypoint เดียวสำหรับทั้งสอง tier = **`takkub qa-gate`** (`--targeted <paths>` = กลางทาง, ไม่ใส่ = full gate; CI/qa pane/มือ user เรียกตัวเดียวกัน, #325) · **specialist ก่อน `takkub done` ใช้ `takkub qa-gate --auto`** (#436): gate เลือก tier จาก `git diff` เอง + พิมพ์เหตุผล — แตะแค่ .css/asset/i18n/md = **ไม่รัน test suite เลย** (Node: typecheck อย่างเดียว) · แตะ logic ของ module = targeted tests ที่ map ได้ · แตะ api/auth/schema/shared/tooling/lockfile = full — **ห้ามรัน full gate เองกับงาน UI/wording ล้วน** · report ของ full gate อยู่ `~/.agent-takkub/runtime/qa-reports/` (ไม่ใช่ใน repo) — ห้ามพิมพ์ `pytest`/`ruff check`/`lint-imports` ดิบเอง
>
