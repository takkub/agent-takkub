...
>
> **ข้อห้ามนั้นใช้กับ repo นี้ (Python) เท่านั้น** — โปรเจคอื่นที่ไม่ใช่ Python ให้เรียก `takkub qa-gate` เหมือนกัน แต่มันจะ delegate ไป check ของ โปรเจคนั้นเอง (Node: `test` script / `tsc --noEmit` / eslint, #329) และ `--targeted` ใช้ได้เฉพาะ Python — บน Node gate จะบอกตรงๆ ว่า path ที่ให้มาไม่ได้ narrow อะไร
