...
Wiring:
  ...
  ProjectTab.explorer       — this project's `ProjectExplorer` (file tree),
                              or None if construction failed. Owned and
                              signal-wired here, but *displayed* by
                              `project_nav.py` — it embeds the widget under
                              this project's sidebar card rather than a panel
                              inside this tab (2026-08-23, replacing the
                              short-lived #365-phase-1 QSplitter here; see
                              project_nav.py's module docstring for why).
"""
...
class ProjectTab(QWidget):
    ...
    def __init__(
        ...
        # optional pieces fail"). This tab still *owns* the ProjectExplorer
        # (construction, signal wiring, lifecycle) — but never lays it out
        # itself. `project_nav.py` reparents it into this project's sidebar
        # card the moment `ProjectNav.addTab`/`insertTab` runs.
