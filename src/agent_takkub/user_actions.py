...
class UserActionsMixin:
    ...
    def _on_resume_clicked(self) -> None:
        """↻ Resume: pop a Qt session picker for the active project's recent
        Lead sessions, then hard-resume the chosen one by respawning the Lead
        pane with ``claude --resume <uuid>`` — the same native path the mobile
        PWA uses (`remote.api.resume_lead` / `remote.notify.
        list_recent_lead_sessions`).

        This replaces the old approach of typing ``/resume`` into the Lead pane
        and driving claude's interactive TUI picker. That path was fragile:
        #113 showed the auto-Enter that submits ``/resume`` lands as the picker
        paints and reads as an empty confirm ("Resume cancelled"), so the
        button was left only *typing* ``/resume`` without submitting — which
        made it feel dead (you pressed the button and nothing happened until
        you pressed Enter yourself in the pane). Driving a Qt picker +
        ``--resume <uuid>`` sidesteps the TUI entirely: one click, pick a
        session, done.

        Multi-provider (#103): ``--resume`` and the JSONL-backed session list
        are claude-CLI specifics. A non-claude Lead simply yields an empty
        list here (graceful "no sessions" message, never a crash) — flagged to
        #103, not solved here, exactly like ``resume_lead``."""
        if not self._btn_resume.isEnabled():
            return
        from datetime import datetime

        from .chatlog_scanner import list_recent_lead_sessions
        from .config import lead_cwd as _lead_cwd
        ...
        try:
            ...
            name = None
        if not name:
            self._status.showMessage("Resume: ไม่มี project ที่เปิดอยู่", 5000)
            return

        self._btn_resume.setEnabled(False)
        try:
            sessions = list_recent_lead_sessions(name)
        except Exception:
            sessions = []
        if not sessions:
            self._btn_resume.setEnabled(True)
            self._status.showMessage(
                f"Resume: ไม่พบ session เก่าของ {name} (Lead ยังไม่เคยคุยในโปรเจคนี้?)",
                6000,
            )
            return
        ...
        for i, sess in enumerate(sessions):
            try:
                ts = datetime.fromtimestamp(sess["mtime"]).strftime("%m-%d %H:%M")
            except (OSError, OverflowError, ValueError, KeyError):
                ts = "??"
            preview = (sess.get("preview") or "").strip() or "(ไม่มี preview)"
            label = f"{i + 1}. {ts} · {preview}"
            labels.append(label)
            by_label[label] = sess.get("uuid", "")

        choice, ok = QInputDialog.getItem(
            self,
            "Resume session",
            f"เลือก session ของ {name} ที่จะ resume\n(Lead pane จะถูก restart เข้าบทสนทนานั้น):",
            labels,
            0,
            False,
        )
        if not ok:
            self._btn_resume.setEnabled(True)
            return
        uuid = by_label.get(choice) or ""
        if not uuid:
            self._btn_resume.setEnabled(True)
            self._status.showMessage("Resume: เลือก session ไม่สำเร็จ", 5000)
            return

        cwd = _lead_cwd(name)
        if not cwd:
            self._btn_resume.setEnabled(True)
            self._status.showMessage(f"Resume: {name} ไม่มี lead cwd", 5000)
            return

        self._btn_resume.setText("⏳ Resuming…")
        try:
            self.orch.close(LEAD.name, project=name, force=True, reason="resume button")
            ok_spawn, msg = self.orch.spawn(LEAD.name, cwd=cwd, project=name, resume_uuid=uuid)
        except Exception as exc:  # never let the slot crash the UI
            ok_spawn, msg = False, str(exc)
        self._btn_resume.setText("↻ Resume")
        self._btn_resume.setEnabled(True)
        if ok_spawn:
            self._status.showMessage(f"↻ Resume: Lead กลับเข้า session {uuid[:8]}… แล้ว", 6000)
            _log_event("resume_button", project=name, session_uuid=uuid)
        else:
            self._status.showMessage(f"Resume ไม่สำเร็จ: {msg or 'spawn failed'}", 7000)
            _log_event("resume_button_failed", project=name, session_uuid=uuid, reason=msg or "")
