...
"""
    ...
    # requests and fans out N instances per role. No hard numeric cap here by
    # design (#2, 2026-07-09 core-upgrade plan) — a machine-derived K would read
    # as an artificial ceiling; the qualitative wave-sequencing advisory below
...
3. **หลาย instance แก้โค้ดใน repo เดียวกัน → ใส่ `--isolation worktree` ทุก instance**
   ...
   **ทีละอัน** (integration จุดเดียวคือ Lead) · repo ไม่ใช่ git → fallback shared
   อัตโนมัติ+เตือน · งาน read-only/คนละ repo → ไม่ต้องใส่
4. **ไม่มี hard cap ตายตัว** — งานอิสระจำนวนมากให้จัดเป็น **waves** ตามน้ำหนักงานต่อ role
   (รอ wave แรก done แล้วค่อย wave ถัด) แทนที่จะยิงทุก feature พร้อมกันทีเดียว — เครื่องแต่ละ
   เครื่องรับไหวไม่เท่ากัน ถ้าเปิดหลาย pane พร้อมกันแล้วเริ่มช้า/ค้าง ให้ลดจำนวนต่อ wave ลง
5. **เฉพาะงาน independent** — งานที่ depend กัน (เช่น backend ต้องเสร็จก่อน frontend ต่อ)
