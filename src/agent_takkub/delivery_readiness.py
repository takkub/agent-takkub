...
def can_accept_input(*, is_ready: bool, account_pending: bool) -> bool:
    """True only when a task is safe to submit into a pane RIGHT NOW.

    Composes the two signals `lead_inbox._send_when_ready`'s poll loop must
    ...
    only *looked* idle. This is the single place that composes both facts,
    so a pane frozen on its provider's own account-pending banner never
    reads as submit-safe no matter how briefly the banner has been up."""
    return bool(is_ready) and not bool(account_pending)
