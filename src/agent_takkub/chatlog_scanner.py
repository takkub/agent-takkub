...
def _first_user_line(path: pathlib.Path) -> str:
    """Best-effort: the first human-typed line in `path`, single-lined and
    truncated. Returns "" on any read/parse failure or if the session has no
    user turn yet — never raises (this feeds a picker, one bad file must not
    break the whole list)."""
    for rec in iter_records(path):
        text = _user_text_only(rec).strip()
        if text:
            return " ".join(text.split())[:_SESSION_PREVIEW_CHARS]
    return ""


def list_recent_lead_sessions(project_ns: str | None, limit: int = 10) -> list[dict]:
    """Recent Lead session JSONLs for `project_ns`'s cwd, newest first, as
    ``[{uuid, mtime, preview}]``. Powers the desktop ↻ Resume picker
    (`UserActionsMixin._on_resume_clicked`).

    Scans every JSONL under the project's cwd-encoded directory (not just the
    currently-open pane's uuid) so a closed or crashed Lead can still be
    resumed. Corrupt/empty files and directories that don't decode to this
    project's cwd are skipped silently — same read-only, best-effort contract
    as the rest of this module.

    Sibling to `remote.notify.list_recent_lead_sessions`, which serves the
    mobile PWA's picker and additionally strips its remote-control prefix and
    hard-caps the limit for data-min. Both resolve sessions the same way
    (cwd-encoded dir under the project's profile); this core copy exists so the
    desktop UI never has to import `agent_takkub.remote` (forbidden by the
    `remote-bolt-on-isolation` import contract).
    """
    ...
    return [
        {"uuid": jsonl.stem, "mtime": mtime, "preview": _first_user_line(jsonl)}
