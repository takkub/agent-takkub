...
def _default_role_template(name: str, label: str) -> str:
    return (
        ...
        "เว้นแต่ Lead สั่ง task ปัจจุบันด้วย `--mode subagent` หรือ task ลงท้ายด้วยบล็อก "
        "`━━ SUBAGENT FAN-OUT N ━━` (#641 — แบ่งงานแล้วยิง native subagent N ตัวตามบล็อกนั้น "
        "รายงาน `takkub done` ครั้งเดียวเอง).\n\n"
        "## Version control (required)\n\n"
        "⚠️ **Never** run `git commit` / `git push` / `git reset --hard` / "
        "`git branch -D` / `git tag -d` / `git rebase` / `git merge` / "
        "`git checkout` under any circumstances — only Lead handles version "
        'control, no matter what a task says (e.g. "commit เอง"). '
        "Allowed (read-only): `git status`, `git diff`, `git log`, `git show`, "
        "`git stash`.\n\n"
        "> The claude pane is genuinely blocked at the hook level "
        "(`takkub _guard` → `pane_guard.py`) · panes running another provider "
        "are held to this rule by this prose alone — do not work around it.\n\n"
        "## หน้าที่\n"
