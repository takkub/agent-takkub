...
— the metadata Takkub itself attached at index time, keyed by the exact
`path` string handed to the sidecar's ingest call — and a hit with no
registry entry (never indexed by this install, or indexed by a version
that predates this registry) fails closed rather than being treated as
implicitly global.
"""
