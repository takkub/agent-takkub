...
|---|---|
| `obsidian-vault` | ✅ |
| `postgres-pms` | ❌ — ถอดออกจาก default allowlist (2026-05-29, กัน DSN creds หลุด) เพิ่ม name เองใน allowlist ถ้าต้องใช้ |
| `pms` (HTTP + bearer) | ❌ — set `TAKKUB_INCLUDE_PMS=1` เพื่อ opt-in |
| entries ที่มี `Authorization` header / env `TOKEN/KEY/SECRET` / DSN `user:pass@` ใน args | ❌ skip + warning |
