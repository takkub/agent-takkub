...
- **หน้าเปิดโปรแกรมไม่ถามอัพเดตอีกเลยหลังเคยติ๊ก "จำตัวเลือกนี้ไว้" + ข้าม (#726):** ค่าที่จำไว้ใช้กับ provider เท่านั้น — ถ้ามี Cockpit เวอร์ชันใหม่จะแสดงหน้าถามอัพเดตเสมอ (เดิมถูกกลืนเงียบ), เพิ่มช่อง "ตอนเปิดโปรแกรม: อัพเดต provider" ใน Settings → Providers ให้เปลี่ยน ถามทุกครั้ง/ข้าม/อัพเดตทั้งหมด ได้จริงตามที่หน้าบูตบอกไว้, และ terminal boot flow แจ้งเมื่อมี Cockpit เวอร์ชันใหม่
...
- **Bulk provider & model selection ("All roles"):** เพิ่ม dropdown ให้เลือก model ควบคู่กับ provider ในแถว "All roles" ของหน้า Settings (Roles Roster) เพื่อให้กดปุ่ม "Apply to all" ครั้งเดียวสามารถเปลี่ยนทั้ง provider และ model ให้กับทุกตำแหน่งในทีมพร้อมกันได้ทันที
...
- **#684 — Project Backlog (popup กลางจอ)**: ที่เก็บงานที่ "เจอแล้วยังไม่ได้สั่งทำ" ข้ามเซสชัน แยกจาก Task
  ...
  checkbox แล้วคลิกการ์ดเพื่อเรียงลำดับ (โชว์เลข ⟨1⟩⟨2⟩) → กด **ตกลง** ส่งรายการตามลำดับเข้า **Lead ให้
  ประเมิน+เลือก role เอง** (เจ้าของสั่งตรงๆ: ไม่มี role picker — "เดี๋ยว lead จัดการ รู้เองว่างานนี้จะสั่งใครทำ")
  Lead สั่งต่อด้วย `takkub backlog assign <id> --role <role>` ซึ่งผูกการ์ดกับ ledger ให้อัตโนมัติ · Lead ยังไม่เปิด
  → คิวไว้ส่งตอน Lead spawn ถัดไป · สถานะครบ: ยังไม่ทำ/กำลังทำ/รอยืนยัน/รอเจ้าของ/ติดอยู่/พักไว้/เสร็จ/ตั้งใจไม่ทำ · เมื่อ pane
...
- **qa-gate smoke-test บน running stack (#475, follow-up #469)** — Node project ที่ **opt-in** ด้วย script `smoke`/`e2e:smoke`/`test:smoke` ใน package.json (root หรือ workspace) → full tier รัน script นั้นหลัง typecheck › test › prisma **เฉพาะเมื่อ stack ขึ้นอยู่แล้ว** (`docker compose ps` มี service running) — ไม่ start stack เอง ไม่มี stack = skip พร้อมบอกเหตุผล · ข้ามถ้า step ก่อนหน้าแดงแล้ว · targeted/style tier ไม่รัน · `TAKKUB_QA_SMOKE=0` ปิด, `TAKKUB_QA_SMOKE_TIMEOUT_S` (default 300) · Python project ยังไม่ทำ
...
- **codex pane โดน boot-stall ceiling fail ทั้งที่ composer พร้อมรับงานแล้ว (#276 รอบ 3)** — พิสูจน์จาก transcript จริง 3 pane คืน 2026-09-02 (reviewer/codex/critic, heartbeat 0.02–0.17s ทุก reprobe): codex-cli 0.152.0 ทิ้งบรรทัด `model: loading` ค้างในกล่อง banner **ตลอดไป** ขณะที่ `directory:`/`permissions:` resolve แล้วและ composer `› Ask Codex to do anything` ตอบสนองจริง → boot-phase marker (#284/#380) เห็น "ยัง boot" ไม่มีวันเคลียร์จน ceiling 300s fail task บน pane ที่ว่างอยู่ · เพิ่ม `shows_stuck_model_field_marker` (จริงเฉพาะเมื่อ `directory:` resolve แล้ว — #380 race ที่ทั้งสอง field loading พร้อมกันยังนับ boot อยู่) + override ใน `_send_when_ready` เมื่อ pane ready จริงและผ่าน settle window 10s เดิมของ codex (`_splash_paste_after_ms` = 0 ใน provider อื่น → no-op) · log `ready_marker_stuck_field_override` · fixture footer ใหม่ "? for shortcuts" · `docs/audit/2026-09-03-issue-276-codex-0152-model-field-stuck.md`
- **gemini/agy pane ติด trust modal ไม่หลุดทั้งที่ ready แล้ว (#330 รอบ 3)** — replay raw bytes ของ `gemini-231930.transcript.log` ด้วย pyte: agy ออกจาก modal ด้วย `CSI ?1049l` แล้ว erase แค่ `CSI J` จาก cursor ลงล่าง → แถว "Do you trust… / Yes, I trust this folder" **ค้างบนจอเหนือ ready banner ตลอด** · `is_at_trust_prompt()` สแกนทั้งจอจึง true ตลอดไป → `_auto_trust` เผา 90s แล้วแจ้ง Lead ว่า pane ยังไม่ได้ใบงาน ทั้งที่ ready ตั้งแต่ ~11s · แก้จุดเดียว: ready prompt ชนะ trust prompt เสมอ (คุ้มทั้ง `_auto_trust` และ `_send_when_ready`) + safety net `_watch_for_late_trust_clear` เฝ้าต่อ 5 นาทีหลัง gave_up ถ้า ready จริง → log `trust_prompt_cleared_late` + resume ส่ง task ที่ค้าง · pre-trust ผ่าน config ของ agy ไม่ทำ (ไม่มี trust field ที่มีเอกสาร — flag #103)
...
- **pane_guard มองไม่เห็นคำสั่งที่ขึ้นต้นด้วย `rtk` และ `git -C <path>` (#466)** — `~/.claude/CLAUDE.md` บังคับทุก pane ใช้ `rtk` prefix แต่ `_CMD_START` ไม่รู้จัก → `rtk git push origin main --force` / `rtk taskkill …` / `rtk pip install -e .` ผ่าน guard มาตลอด (host_destructive #169, host_network #400, pip_editable #202, git_lead_only #314/#438) · `_GIT_SUBCMD_GAP` ไม่ข้าม `-C <value>` → `git -C . reset --hard` ผ่านเช่นกัน · แก้ที่ regex root กระทบทุก rule · ทุกครั้งที่ deny log event `pane_guard_denied` {role, project, rule, cmd[:200]} ลง events.log (`takkub ma` นับ) · deny message ของ push ใน worktree บอกรูปแบบที่ผ่านได้ตรงๆ (`git push -u origin wt/<role>-<ts>`)
...
- **proactive `/compact` ยิงใส่ Lead ระหว่างรอ specialist ทำงาน (#465)** — `_check_proactive_compact` เดิมนับ Lead "idle" แค่จากหน้าจอ ready prompt เฉยๆ ทั้งที่ Lead กำลัง orchestrate อยู่ (specialist ยัง `working`, มี `takkub wait` ค้าง, หรือ inbox digest ยังไม่ถึง pane) — พิสูจน์จริงจาก `runtime/events.log` 09:39:26/10:36:31 ที่ Lead โดน compact ระหว่าง backend ทำ #463/#464 อยู่ · ตอนนี้ Lead ต้องทั้งโปรเจคนิ่งจริง (ไม่มี pane `working`, ไม่มี wait ค้าง, ไม่มี digest ค้าง) ก่อน idle clock จะเริ่มนับ, และ specialist ที่เข้า tier "waiting-lead" (#463 — turn จบจริงระหว่างรอ Lead ตอบ) ก็ไม่นับ idle เหมือนกัน (`proactive_idle_compact_skipped` log reason `lead-specialist-working`/`lead-wait-pending`/`lead-inbox-unread`/`waiting-lead`) · เจอบั๊กเสริมระหว่างตรวจ gate "nothing new": การ skip เดิม stamp `sent_ts` ตัวเดียวกับตอนยิงจริง ทำให้ episode เดียวกันยิงไม่ได้อีกเลยแม้ byte ใหม่จะมาครบตามเกณฑ์ทีหลัง (ขัดกับ comment เดิมที่บอกว่า "ยิงใหม่ได้เมื่อมีของจริง") — แก้แล้วไม่ stamp sent_ts บน skip นี้อีก dedupe การ log ด้วย field แยกแทน · TTL จริงของ prompt-cache คือ ~1h ปกติ/ ~5min ตอน account เข้า usage-overage (`limit_status.is_in_overage`) ไม่ใช่ 25min ตายตัว — เปลี่ยน default เป็น 55min (ใกล้ TTL ปกติแต่ไม่ชนบ่อย, ประหยัดกว่า compact ทิ้งบ่อยเกินจำเป็น) และเพิ่ม threshold แยก 4min เฉพาะตอน overage (`TAKKUB_PROACTIVE_COMPACT_OVERAGE_IDLE_AFTER_S`, อ่าน usage state ที่ cache ไว้ข้าม process อยู่แล้ว ไม่ยิง network เพิ่ม) — ปิดได้ด้วย `=0` ทั้งสองตัวแยกกัน
- **`takkub status`/`list` โชว์ pane "working" ค้างหลัง turn จบแล้ว (#463)** — claude 2.1.252 ทิ้ง spinner text (`✽ Mulling… running stop hook · Ns`) ค้างบนจอหลัง Stop hook จบ ทำให้ PTY-scrape ยึดว่ายังทำงาน · เพิ่ม display tier `waiting-lead`: `PaneState.last_turn_end_ts` stamp โดย `consume_pane_hook` เฉพาะตอน Stop hook ผ่าน (ไม่ block) และต้อง `>= blocked_on_lead_ts` (จาก `takkub progress`/`send --to lead`) — progress กลางงานที่ยังทำต่อไม่นับ · Lead `send` เข้า pane/spawn ใหม่ล้าง flag · `progress()` mark delivery RUNNING ทันที ตัด notice `delivery-superseded` ปลอมตอน Lead ส่ง GO (#255/#392) · provider ไม่มี Stop hook (codex/gemini-agy/opencode) ไม่แตะ ตก scrape เดิม (claude-only by construction, #103) · e2e เจอต่อ: Lead `send` เข้า pane ที่ delivery RUNNING แล้ว ยังถูก `supersede_for_session` cancel (has_reached_pane ถือ RUNNING = reached) → `done()` หา delivery ไม่เจอ ledger จบ CANCELLED — เกณฑ์ cancel เปลี่ยนเป็น `_RESEND_ELIGIBLE_STATES` (WRITING/WRITTEN/SUBMITTING/ACCEPTED) RUNNING/SPAWNED_IDLE ปล่อยให้ `done()` ปิดเอง
